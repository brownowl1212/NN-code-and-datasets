...
# the CALCE dataset. This code trains and test the model.
...
whosmodel = 'improved' # Selects the model to train and test: improved or original
dataset = 'CALCE' # Selects the datset to train and test: CALCE, CAS, Oxford

# Record the start time of the script 
start_time = time.time()

# Paths to the directories containing the dataset.
location_CALCE = ''
location_CAS = ''
location_Oxford = ''

# Load data: Importing battery charge curves from specified location.
if dataset == 'CALCE':
    ...
    curve_cell_1 = np.genfromtxt(location_CAS + 'batt07_cccv.txt', delimiter=',')

elif dataset == 'Oxford':
    curve_cell_1 = np.genfromtxt(location_Oxford + 'Cell1.txt', delimiter=',')
    curve_cell_2 = np.genfromtxt(location_Oxford + 'Cell3.txt', delimiter=',')
    curve_cell_3 = np.genfromtxt(location_Oxford + 'Cell7.txt', delimiter=',')
    curve_cell_4 = np.genfromtxt(location_Oxford + 'Cell8.txt', delimiter=',')

if dataset == 'CALCE' or dataset == 'Oxford':
    # Dividing the dataset into testing and training data.
    curve_train = [curve_cell_3]
    curve_test = [curve_cell_4, curve_cell_1, curve_cell_2]

elif dataset == 'CAS':
    # Divides the CAS charge cycles into two - test data and as training data
    curve_train = curve_cell_1[0:-1:2]
    curve_test = curve_cell_1[1:-1:2] 
...
if train == 1:
    if whosmodel == 'improved':
        # Improved model architecture
        model = Sequential([
            Conv1D(filters=224, kernel_size=15, activation='elu', padding='causal', input_shape=(None, 2)),
            MaxPooling1D(2),
            Conv1D(filters=256, kernel_size=7, activation='elu', padding='causal'),
            MaxPooling1D(2),
            GlobalMaxPooling1D(),
        ...
        # Optimiser setup
        adam = Adam(learning_rate=0.000824966994861931)
    ...
    if whosmodel == 'improved':
        filepath = "models_improved/{epoch:02d}-{val_loss:.2f}.hdf5"
    else:   
    ...
    history = model.fit(
        ...
        Output_train,
        epochs=2000,
        validation_split=0.4,  # % of data used for validation
...
if showplot == 1:
    ...
    for idx, i in enumerate(selected_indices, start=1):
        ...
        # Plotting the predicted values (assuming predicted_test has the same structure as Output_test)
